    ...
    public class DynamicOctree {
        ...
        public void draw() {
            foreach (long node_id in base_nodes.Keys) {
                Draw3D.text_3D(EngineState.spritebatch, $"ID {node_id.ToString()}\n{base_nodes[node_id].values.Count.ToString()} objects" , "pf", base_nodes[node_id].bounds.Min + ((base_nodes[node_id].bounds.Max - base_nodes[node_id].bounds.Min) / 2), -EngineState.camera.direction, 15f, Color.Black);
        ...
        long create_base_node((int X, int Y, int Z) pos) {
            return create_base_node(pos.X, pos.Y, pos.Z);
        }
        long create_base_node(int X, int Y, int Z) {
