...
namespace Magpie.Engine.Collision {
    public class MixedCollision {
        ...
        public struct gjk_simplex {
            ...
            public (Vector3 A, Vector3 B) supports_A;
            public (Vector3 A, Vector3 B) supports_B;
            public (Vector3 A, Vector3 B) supports_C;
            public (Vector3 A, Vector3 B) supports_D;

            public float barycentric_A;
            public float barycentric_B;
            public float barycentric_C;
            public float barycentric_D;

            public Vector3 minkowski;
            public Vector3 minkowski_world;
            ...
            public gjk_simplex() {
                ...
                supports_A = (Vector3.Zero, Vector3.Zero);
                supports_B = (Vector3.Zero, Vector3.Zero);
                supports_C = (Vector3.Zero, Vector3.Zero);
                supports_D = (Vector3.Zero, Vector3.Zero);

                support_A = Vector3.Zero;
                support_B = Vector3.Zero;

                
                minkowski = Vector3.Zero;
                minkowski_world = Vector3.Zero;
            ...
            public gjk_simplex copy() {
                return new gjk_simplex() {
                    ...
                    barycentric_A = barycentric_A,
                    barycentric_B = barycentric_B,
                    barycentric_C = barycentric_C,
                    barycentric_D = barycentric_D,
                    supports_A = supports_A,
                    supports_B = supports_B,
                    supports_C = supports_C,
        ...
        public struct collision_result {
            ...
            public void draw(Vector3 world_pos) {
                ...
                //if (draw_simplex > simplex_list.Count-1)
                //    draw_simplex = simplex_list.Count - 1;
                if (simplex_list != null && draw_simplex > -1 && draw_simplex < simplex_list.Count) {
                    ...
                    Draw3D.text_3D(EngineState.spritebatch,
                        $"{draw_simplex + 1}/{simplex_list.Count} {simplex.stage.ToString()} {simplex.hit} [{intersects}]\n" +
                        $"dist {simplex.distance}\n" +
                        $"SA {simplex.support_A} \nSB {simplex.support_B}" +
                    ...
                    Draw3D.xyz_cross(
                        Vector3.Transform(Vector3.Zero, (simplex.A_transform)),
                        5f, Color.Orange);

                    Draw3D.xyz_cross(
                        Vector3.Transform(Vector3.Zero, (simplex.B_transform)),
                        5f, Color.Orange);

                    Draw3D.xyz_cross(simplex.A_transform.Translation - simplex.B_transform.Translation,
                        2f, Color.Green);

                    Draw3D.line(
                        simplex.A_transform.Translation,
                        simplex.A_transform.Translation + Vector3.Normalize(Vector3.Transform(simplex.direction, Matrix.Invert(simplex.A_transform))),
                        Color.Red);

                    Draw3D.line(
                        simplex.B_transform.Translation,
                        simplex.B_transform.Translation + Vector3.Normalize(Vector3.Transform(-simplex.direction, Matrix.Invert(simplex.B_transform))),
                        Color.Green);

                    Draw3D.line(
                        simplex.A_transform.Translation,
                        simplex.A_transform.Translation + Vector3.Normalize(simplex.direction),
                        Color.HotPink);

                    Draw3D.line(
                        simplex.B_transform.Translation,
                        simplex.B_transform.Translation + Vector3.Normalize(simplex.direction),
                        Color.HotPink);

                    if (simplex.stage == simplex_type.point) {
                        //Draw3D.xyz_cross(world_pos + (simplex.A), 0.1f, Color.Red);

                        Draw3D.xyz_cross(simplex.supports_A.A, 0.2f, Color.Green);
                        Draw3D.xyz_cross(simplex.supports_A.B, 0.2f, Color.ForestGreen);

                        Draw3D.arrow(simplex.A, simplex.A + (simplex.direction) * 0.5f, 0.3f, Color.HotPink);
                        Draw3D.xyz_cross(simplex.A, .5f, Color.Red);


                    } else if (simplex.stage == simplex_type.line) {
                        //Draw3D.arrow(world_pos + simplex.A, world_pos + simplex.B, 0.1f, Color.HotPink);
                        Draw3D.line(simplex.A, simplex.B, Color.HotPink);

                        Draw3D.xyz_cross(simplex.supports_B.A, 0.2f, Color.DeepPink);
                        Draw3D.xyz_cross(simplex.supports_B.B, 0.2f, Color.ForestGreen);

                        Draw3D.line(simplex.supports_A.A, simplex.supports_B.A, Color.Green);
                        Draw3D.line(simplex.supports_A.B, simplex.supports_B.B, Color.ForestGreen);
                        //Draw3D.xyz_cross(world_pos + simplex.A, 0.1f, Color.Red);
                        //Draw3D.xyz_cross(world_pos + simplex.B, 0.1f, Color.Green);

                        Draw3D.xyz_cross(simplex.A, .5f, Color.Red);
                        Draw3D.xyz_cross(simplex.B, .5f, Color.Green);

                        Draw3D.arrow(
                            ((simplex.A + simplex.B) / 2),
                            ((simplex.A + simplex.B) / 2) + (simplex.direction) * 0.5f,
                            0.3f, Color.HotPink);
                        ...
                        Draw3D.lines(Color.HotPink, simplex.A, simplex.B, simplex.C, simplex.A);

                        Draw3D.xyz_cross(simplex.supports_C.A, 0.2f, Color.DeepPink);
                        Draw3D.xyz_cross(simplex.supports_C.B, 0.2f, Color.ForestGreen);

                        Draw3D.lines(Color.Green, simplex.supports_A.A, simplex.supports_B.A, simplex.supports_C.A);
                        Draw3D.lines(Color.ForestGreen, simplex.supports_A.B, simplex.supports_B.B, simplex.supports_C.B);

                        //Draw3D.xyz_cross(world_pos + simplex.A, 0.1f, Color.Red);
                        //Draw3D.xyz_cross(world_pos + simplex.B, 0.1f, Color.Green);
                        //Draw3D.xyz_cross(world_pos + simplex.C, 0.1f, Color.Blue);

                        Draw3D.xyz_cross(simplex.A, 1f, Color.Red);
                        Draw3D.xyz_cross(simplex.B, 1f, Color.Green);
                        Draw3D.xyz_cross(simplex.C, 1f, Color.Blue);

                        var N = Vector3.Normalize(Vector3.Cross((simplex.B - simplex.A), (simplex.C - simplex.A)));

                        Draw3D.arrow(
                            ((simplex.A + simplex.B + simplex.C) / 3),
                            ((simplex.A + simplex.B + simplex.C) / 3) + N, 0.3f, Color.Red);

                        Draw3D.arrow(
                            ((simplex.A + simplex.B + simplex.C) / 3),
                            ((simplex.A + simplex.B + simplex.C) / 3) + (simplex.direction) * 0.5f, 0.3f, Color.HotPink);
                        ...
                        Draw3D.xyz_cross(simplex.supports_D.A, 0.2f, Color.DeepPink);
                        Draw3D.xyz_cross(simplex.supports_D.B, 0.2f, Color.ForestGreen);

                        Draw3D.lines(Color.Green, simplex.supports_A.A, simplex.supports_B.A, simplex.supports_C.A, simplex.supports_D.A);
                        Draw3D.lines(Color.ForestGreen, simplex.supports_A.B, simplex.supports_B.B, simplex.supports_C.B, simplex.supports_D.B);
                        ...
                        Draw3D.lines(Color.HotPink, simplex.A, simplex.B, simplex.C, simplex.A);
                        Draw3D.line(simplex.A, simplex.D, Color.HotPink);
                        Draw3D.line(simplex.B, simplex.D, Color.HotPink);
                        Draw3D.line(simplex.C, simplex.D, Color.HotPink);
                        Draw3D.xyz_cross(simplex.A, 1f, Color.Red);
                        Draw3D.xyz_cross(simplex.B, 1f, Color.Green);
                        Draw3D.xyz_cross(simplex.C, 1f, Color.Blue);
                        Draw3D.xyz_cross(simplex.D, 1f, Color.Yellow);
                    }
                }
            }
        ...
        //}
        static bool same_dir(Vector3 A, Vector3 B) => Vector3.Dot(A, B) >= 0f;
        static void swap(ref Vector3 A, ref Vector3 B)  {
            Vector3 tmp = A;
            A = B; B = tmp;
        }

        static void swap(ref (Vector3,Vector3) A, ref (Vector3, Vector3) B) {
            var tmp = A;
            A = B; B = tmp;
        }
        ...
        public static collision_result intersects(Shape3D shape_A, Shape3D shape_B, Matrix w_a, Matrix w_b) {
            ...
            var dir = (w_b.Translation - w_a.Translation);

            simplex.direction = Vector3.One;// (Vector3.Transform(dir, Matrix.Invert(w_b) * Matrix.Invert(w_a))); 
            ...
            while (iteration < max_iterations) {
                simplex.support_A = Vector3.Transform(shape_A.support(Vector3.Transform( simplex.direction, Matrix.Invert(w_a)), Vector3.Zero), w_a);
                simplex.support_B = Vector3.Transform(shape_B.support(Vector3.Transform(-simplex.direction, Matrix.Invert(w_b)), Vector3.Zero), w_b);
               

                //simplex.support_A_world = (Vector3.Transform(simplex.support_A, (w_a )));
                //simplex.support_B_world = (Vector3.Transform(simplex.support_B, (w_b )));

                //simplex.support_A = (Vector3.Transform(simplex.support_A, (w_a)));
                //simplex.support_B = (Vector3.Transform(simplex.support_B, (w_b)));

                Vector3 minkowski = simplex.support_A - simplex.support_B;

                simplex.minkowski = minkowski;

                if (simplex.stage == simplex_type.empty) {
                    simplex.A = minkowski;
                    simplex.supports_A = (simplex.support_A, simplex.support_B);

                    simplex.stage = simplex_type.point;

                    simplex.early_exit_reason = "Starting point";
                    ...
                    simplex.B = minkowski;
                    simplex.supports_B = (simplex.support_A, simplex.support_B);

                    simplex.stage = simplex_type.line;

                    result.simplex_list.Add(simplex.copy());

                    if (Vector3.Distance(Vector3.Zero, CollisionHelper.point_of_minimum_norm(simplex.A, simplex.B, Vector3.Zero)) < CollisionHelper.epsilon) {
                        result.intersects = true;
                        simplex.hit = result.intersects;
                        simplex.early_exit_reason = "Found point on line";
                        result.simplex_list.Add(simplex.copy());
                        break;
                    }
                    ...
                    var AB = simplex.B - simplex.A;
                    var BA = simplex.A - simplex.B;

                    // BA is the same direction as BO, meaning the point we just found (B) is on the other side of origin from A
                    // so go find a tri in dir BAxBOxBA
                    if (same_dir(BA, -simplex.B)) {


                        simplex.barycentric_A = Vector3.Dot(simplex.B, BA);
                        simplex.barycentric_B = Vector3.Dot(simplex.A, AB);

                        simplex.direction = Vector3.Cross(Vector3.Cross(BA, -simplex.B), BA);

                        //swap(ref simplex.A, ref simplex.B);
                        //swap(ref simplex.supports_A, ref simplex.supports_B);

                        simplex.early_exit_reason = "BA same direction as BO";
                        result.simplex_list.Add(simplex.copy());
                        simplex.early_exit_reason = "";

                        //otherwise B is closer to origin than A, so make B the new A and try to find a new point in the other direction
                    } else {
                        simplex.A = simplex.B;
                        simplex.supports_A = simplex.supports_B;

                        simplex.B = Vector3.Zero;
                        simplex.B= Vector3.Zero;
                        simplex.supports_B = (Vector3.Zero, Vector3.Zero);

                        simplex.stage = simplex_type.point;
                        simplex.early_exit_reason = "B past origin from A";
                    ...
                    simplex.supports_C = (simplex.support_A, simplex.support_B);

                    simplex.stage = simplex_type.triangle;

                    simplex.early_exit_reason = "Create Tri";
                    result.simplex_list.Add(simplex.copy());

                    /*
                    if (Vector3.Distance(Vector3.Zero, CollisionHelper.closest_point_on_triangle_slow(simplex.A, simplex.B, simplex.C, Vector3.Zero)) < CollisionHelper.epsilon) {

                        result.intersects = true; 
                        simplex.hit = result.intersects;
                        simplex.early_exit_reason = "hit";
                        result.simplex_list.Add(simplex.copy());
                        simplex.early_exit_reason = "";
                        return result;
                    } 
                    */
                    ...
                    var N = Vector3.Cross((simplex.B - simplex.A), (simplex.C - simplex.A));
                    var AC = simplex.C - simplex.A;
                    var CA = simplex.A - simplex.C;
                    var CB = simplex.B - simplex.C;
                    var BA = simplex.A - simplex.B;
                    var BC = simplex.C - simplex.B;

                    if (same_dir(Vector3.Cross(N, AC), -simplex.C)) {
                        if (same_dir(AC, -simplex.C)) {
                            simplex.early_exit_reason = "AC->";
                            result.simplex_list.Add(simplex.copy());
                            simplex.early_exit_reason = "";

                            simplex.B = simplex.C;
                            simplex.supports_B = simplex.supports_C;
                            simplex.C = Vector3.Zero;
                            simplex.supports_C = (Vector3.Zero, Vector3.Zero);

                            simplex.stage = simplex_type.line;

                            BA = simplex.B - simplex.A;
                            simplex.direction = Vector3.Cross(Vector3.Cross(BA, -simplex.A), BA);
                            ...
                            if (same_dir(CB, -simplex.C)) {
                                simplex.early_exit_reason = "CB1->";
                                result.simplex_list.Add(simplex.copy());
                                simplex.early_exit_reason = "";

                                simplex.A = simplex.C;
                                simplex.supports_A = simplex.supports_C;
                                simplex.C = Vector3.Zero;
                                simplex.supports_C = (Vector3.Zero, Vector3.Zero);
                        ...
                        if (same_dir(Vector3.Cross(BC, N), -simplex.C)) {
                            if (same_dir(CB, -simplex.C)) {
                                simplex.early_exit_reason = "CB2->";
                                result.simplex_list.Add(simplex.copy());
                                simplex.early_exit_reason = "";
                    ...
                    simplex.D = minkowski;
                    simplex.supports_D = (simplex.support_A, simplex.support_B);

                    simplex.stage = simplex_type.tetrahedron;

                    var AC = simplex.C - simplex.A;
                    var AB = simplex.B - simplex.A;
                    var AD = simplex.D - simplex.A;
                    var BC = simplex.C - simplex.B;
                    var BD = simplex.D - simplex.B;

                    var ABC = Vector3.Cross(AB, AC);
                    var ACD = Vector3.Cross(AD, AC);
                    var ABD = Vector3.Cross(AB, AD);
                    var BCD = Vector3.Cross(BC, BD);

                    simplex.early_exit_reason = "Create tet";
                    result.simplex_list.Add(simplex.copy());
                    ...
                    if (CollisionHelper.point_inside_tetrahedron(simplex.A, simplex.B, simplex.C, simplex.D, Vector3.Zero) ) {
                        ...
                        simplex.early_exit_reason = "job cun";
                        result.simplex_list.Add(simplex.copy());
                        simplex.early_exit_reason = "";
                        break;
                    }
                    ...
                    if (same_dir(BCD, -simplex.D)) {

                        simplex.early_exit_reason = "BCD->";
                        result.simplex_list.Add(simplex.copy());
                        simplex.early_exit_reason = "";

                        simplex.direction = BCD;
                        //simplex.direction = Vector3.Cross(Vector3.Cross(BC, -simplex.D), BC);
                        simplex.D = simplex.A;
                        simplex.supports_D = simplex.supports_A;
                    ...
                    else if (same_dir(ACD, -simplex.B)) {

                        simplex.early_exit_reason = "ACD->";
                        result.simplex_list.Add(simplex.copy());
                        simplex.early_exit_reason = "";

                        simplex.direction = ACD;

                        simplex.B = simplex.D;
                        simplex.supports_B = simplex.supports_D;
                    ...
                    }else if (same_dir(ABD, -simplex.D)) {

                        simplex.early_exit_reason = "ABD->";
                        result.simplex_list.Add(simplex.copy());
                        simplex.early_exit_reason = "";

                        simplex.direction = ABD;

                        simplex.C = simplex.D;
                        simplex.supports_C = simplex.supports_D;
                    ...
                    } else if(same_dir(ABC, -simplex.D)) {
                        ...
                        simplex.D = Vector3.Zero;
                        simplex.supports_D = (Vector3.Zero, Vector3.Zero);
            ...
            switch (simplex.stage) {
                case simplex_type.empty: break;
                case simplex_type.point:
                    result.closest_point_A = simplex.supports_A.A;
                    result.closest_point_B = simplex.supports_A.B;
                    result.distance = Vector3.Distance(result.closest_point_A, result.closest_point_B);
                    break;
                case simplex_type.line:
                    break;
                case simplex_type.triangle:
                    break;
                case simplex_type.tetrahedron:
                    break;
