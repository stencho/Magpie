...
{
    PSO output = (PSO) 0;
    output.Normals = tex2D(NORMAL, input.ScreenPosition.xy);
    output.Diffuse = tex2D(DIFFUSE, input.ScreenPosition.xy);
    
    output.Depth = tex2D(DEPTH, input.ScreenPosition.xy);
